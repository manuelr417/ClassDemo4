...
$(document).on('pagebeforeshow', "#cars", function( event, ui ) {
	...
	$.ajax({
		...
		success : function(data, textStatus, jqXHR){
			...
			for (var i=0; i < len; ++i){
				car = carList[i];
				list.append("<li><a onclick=GetCar(" + car.cid + ")>" + 
					"<h2>" + car.cbrand + " " + car.cmodel +  "</h2>" + 
					"<p><strong> Year: " + car.cyear + "</strong></p>" + 
					"<p>" + car.cdesc + "</p>" +
					"<p class=\"ui-li-aside\">" + accounting.formatMoney(car.cprice) + "</p>" +
					"</a></li>");
			}
...
function convert(dbModel){
	...
	cliModel.id = dbModel.cid;
	cliModel.make = dbModel.cbrand;
	cliModel.model = dbModel.cmodel;
	cliModel.year = dbModel.cyear;
	cliModel.description = dbModel.cdesc;
	cliModel.price = dbModel.cprice;
	return cliModel;
}
function SaveCar(){
	$.mobile.loading("show");
...
function GetCar(id){
	$.mobile.loading("show");
	$.ajax({
		url : "http://localhost:3412/ClassDemo4Srv/cars/" + id,
		method: 'get',
		contentType: "application/json",
		dataType:"json",
		success : function(data, textStatus, jqXHR){
			currentCar = convert(data.car);
			$.mobile.loading("hide");
